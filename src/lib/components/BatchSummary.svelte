<script lang="ts">
	...
	import { decidingProbability, verdictLabel } from '$lib/batch-display';
	import type { BatchJudgeResponse } from '$lib/types/batch';

	let { response }: { response: BatchJudgeResponse } = $props();
	...
	const byVerdict = $derived(
		[...new Set(response.results.map((result) => result.verdict))]
			.map((verdict) => ({
				verdict,
				count: response.results.filter((result) => result.verdict === verdict).length
			}))
	...
	const bands = $derived.by(() => {
		const values = response.results.map((result) =>
			decidingProbability(response.theme, result.signals)
		);
		return [
	...
	const graded = $derived(response.results.filter((result) => result.gold !== undefined));
			...
			<ul class="mt-1.5 space-y-1">
				{#each byVerdict as row (row.verdict)}
					<li class="flex items-center gap-2 text-sm">
						<span class="w-40 shrink-0">{verdictLabel(row.verdict)}</span>
