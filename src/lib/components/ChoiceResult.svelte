...
<p class="mt-1">
	<span
		class="inline-block rounded-full bg-neutral-900 px-3 py-1 text-sm font-semibold text-white dark:bg-neutral-100 dark:text-neutral-900"
	>
		{selectedLabel}
	</span>
</p>
...
<ul class="mt-3 space-y-1.5">
	{#each visible as option (option.key)}
		<li>
			...
			<div class="mt-1 h-1.5 w-full rounded-full bg-neutral-200 dark:bg-neutral-700">
				<div
					class="h-1.5 rounded-full bg-neutral-800 dark:bg-neutral-300"
					style="width: {toPercent(option.probability)}%"
				></div>
			</div>
		</li>
	{/each}
