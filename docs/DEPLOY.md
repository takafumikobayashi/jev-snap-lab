...
| `BATCH_JUDGE_ENABLED` | `true` | 任意 | BATCH JUDGEを有効にする。未設定なら `/batch` が404になり、`/api/batch` も応答しない。明示的な `true` だけを有効とする。1回の判定は50件で0.07〜0.18円（実測、[BATCH_JUDGE_DESIGN.md](BATCH_JUDGE_DESIGN.md) §4.6） |
