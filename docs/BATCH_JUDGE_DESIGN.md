...
| 結果のまとめ | 結論の内訳、判定に効いた確率の分布（**0.4〜0.7を「揺れやすい」として出す**）、ラベルがあるときは照合と食い違いの内訳 |
...
**採用方式は3テーマとも単一の結論である。** PRIVACY は3つのNoulから `no_signal` / `review` を出し、DEADLINE と DX JUDGE は Choice をそのまま使う。Pattern C で独立Noulへ展開した構成も比較したが、どちらも一致率が落ちた（§4.6）。

したがって表に出すのは単一の一致率でよい。ただし内訳は内部で見られるようにする。

| テーマ | 内訳として見るもの |
|---|---|
| PRIVACY | **見逃しと過検知を分ける。** 個人情報を見落とすのと、安全な文を要確認にするのとでは重さが違う。判定に使わない `sensitive` も理由として持つ |
| DEADLINE / DX | **混同の中身。** `bpr→digital` のようにどちらへ寄ったかが分からないと直せない |

UIが複雑になるなら詳細はdebug表示でよい。
