...
画面には次を**入力欄の手前に**常時出す。判定結果と一緒に出しても遅い。**画面に残す注意書きは、3テーマを通してこれ1つだけである。** テーマごとの注記も置かない。
...
この制約は**画面に注意文として出さない。** 判定が `いますぐ / 今日中 / 近いうち / 先の話 / 期限なし` という粗い区分である以上、ラベル自体が厳密な日時でないことを示している（§6）。
...
これも**画面に注意文として出さない。** 3択のチップをコンサルティング判断と取る余地は無い。CITY の「実在の自治体の公式案内ではありません」は誤認の余地が本当にあるので必要だが、これは違う（§6）。
