...
架空版は `scripts/build-fictional-city.mjs` が実データから機械的に生成する。実行は `pnpm city:build data/city/local-<自治体>-<施行日>.json`。対応表と出力先は既定値を持つが、元データのパスは持たない（`package.json` に書くと自治体名がコミット対象へ載る）。組織単位47件・分掌事務615件・候補28件という構造をそのまま保つため、ルーティング精度は落ちない。対応表（`scripts/*.local.json`）もリポジトリに含めない。
...
代わりに、**元データから固有名詞の候補を機械的に列挙し、すべてが分類済みであることを要求する**。分類は対応表の `replacements`（置換する＝識別情報）か `allowGeneric`（一般語＝残してよい）のどちらかに入れる。未分類が1件でもあれば `pnpm city:build` が異常終了するので、データ更新で新しい固有名詞が入っても気付ける。候補は地名接尾辞、団体名接尾辞、カタカナ語、数字を含む名称のパターンで拾う。地名接尾辞は日本の行政区分をすべて並べる（`市区町村都道府県郡`）。`市町村県` だけにしていたため特別区・府・道・郡の地名が素通りしていた。下水道・林道・財産区のような一般語も同じパターンに当たるが、取りこぼすより分類を要求するほうが安全側に倒れる。
...
1と3の判定に「候補が既知の語を含むか」を使ってはいけない。候補の正規表現は貪欲なので、1つのマッチが複数の固有名詞をまたぐ。`青空市から未分類市` は丸ごと1件のマッチになり、既知の `青空市` を含むという理由で分類済みと見なされ、`未分類市` が素通りしていた。**既知の語を空白へ落としてから、残りかすで候補を拾い直す。** 空白は候補パターンの文字クラスから除外されているため、語の区切りとして働く。

この結果、すでに置換済みの名前の「一般的な後半部分」（`H県国民健康保険団体連合会` の `国民健康保険団体連合会` など）も分類を要求される。識別子を含まないので `allowGeneric` へ登録する。手間は増えるが、そこに未知の地名が紛れていれば必ず落ちる。

**3段すべてを通ってから出力する。** 以前は生成直後に書き出しており、未分類の固有名詞で異常終了しても追跡対象の `fictional-m-city.json` が未検証のデータで上書きされていた。書き込みは同じディレクトリの一時ファイルへ行い、`rename` で置き換える。失敗時は出力先に触れない。

後段の `pnpm city:check-leak` はこの取りこぼしを拾えない。対応表の `replacements` と `forbiddenTerms` に載っている語しか探さないため、**未分類の語は原理的に検出できない**。`city:build` で止めるのが唯一の砦になる。

その `pnpm city:check-leak` は、**コミット対象とビルド成果物**を検査する。`publicSummary` はJevの criteria としてリクエストに載るため、漏れれば画面だけでなく上流APIにも渡る。公開前に必ず実行する。
