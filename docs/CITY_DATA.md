...
組織ページの `publicSummary` だけでは、条文に明記された事務がJevへ届かない。実例として、政策企画課の市民向け説明には「サンフレッチェ広島応援事業」が現れないため、この入力で政策企画課が上位に来ず「絞り込めない 77%」「教育委員会 21%」となった。Jevは一般常識で答えており推論として妥当だが、根拠データを持っているのに渡していなかったこちら側の問題である。

分掌事務を載せた後は政策企画課98%になる。

### 載せる分掌の選び方

全件は載せない。件数は課ごとに0〜69件と偏りが大きく、トークン量も増えるため、1候補あたり12件を上限とする。

上限で切る以上、条文順の先頭から詰めると後ろの具体的な分掌が落ちる。実際、政策企画課では「サンフレッチェ広島応援事業」が14番目にあり、抽象的な計画・調整の分掌に押し出されていた。次の優先順位で選ぶ。
