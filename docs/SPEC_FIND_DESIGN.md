...
4. 抜粋は600字以内で、区切りのよいところまでにする。区切りは**括弧の外にある句点**か**箇条書きの項目の先頭**。仕様書には「◯◯（……をいう。以下同じ。）」という定義が多く、括弧内の句点で切ると閉じ括弧と後続が失われる。§1.3 のようにほぼ箇条書きで句点が冒頭に1つしか無い節もあるため、項目の切れ目も区切りとして使う。
...
| `MAX_CORPUS_CHARS` | 20,000字 | **本文と見出しの合計。** `state` と最長の質問で32k tokensという制限に対し、約66%に収める |

**件数と1件あたりの上限だけでは、リクエスト全体の大きさが決まらない。** 40件 × 600字を上限まで使うと約25,700字になり、`state` だけで32kの約85%に達する。測っていない領域へ入るため、総量でも止める（[IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md) §8.1）。

現在の配布コーパスは15,612字で、総量上限の78%である。文字数は `countCodePoints` で数える（入力検証と同じ）。
...
## 10. v1: Standard Specification Navigator（設計段階）

v0は単一PDFの検索である。v1では、**現在の資料に答えが無い場合を区別できる**ようにし、機能要件Excelを第2の層として足す。

### 10.1 解決したい問題

最大の課題はtoken量ではなく、**ユーザーの具体的な質問にPDFが答えを持っていないこと**である。

| 入力 | 答えがある資料 |
|---|---|
| データを分析したい | 共通機能標準仕様書（PDF） |
| 条件を指定してデータを出力したい | 機能要件（Excel） |
| この項目を保持できる？ | 項目定義書 |
| APIでどう連携する？ | API仕様書 |

これをJevの検索精度の問題として扱わない。3種類を明確に区別する。

| 種別 | 意味 |
|---|---|
| **Retrieval Gap** | 正解はcorpusにあるが、Jevが見つけられない |
| **Corpus Gap** | 元資料にはあるが、代表passageの抽出で落としている |
| **Source Gap** | 検索している資料そのものに答えが無い |

**Source Gap を retrieval failure として扱わない。** 見つからなかったのではなく、そこには書いていない。

### 10.2 情報アーキテクチャ

```text
「やりたいこと」
  -> Layer 1  共通機能標準仕様書（PDF）   関連する機能領域を見つける
  -> Layer 2  機能要件（Excel）           具体的な要件IDと要件文
  -> Layer 3  項目定義 / API / 連携仕様   （v1では設計のみ。実装しない）
```

v1はLayer 2までとする。

### 10.3 機能要件Excelの実測（2026-09-21）

第2.7版のExcelを取得して構造を確認した。**設計の前提が2つ崩れる。**

- 公式ページの表記は**「機能要件（第2.7版）」**。PDF本文は「別紙1_機能要件」と呼ぶが、ページ上にその名称のファイルは無い。出典表示はページ側の表記に合わせる
- ファイルは `common_05.xlsx`（274KB）。シート「共通機能」に**761件**の要件行

列構成:

| 列 | 内容 | 例 |
|---|---|---|
| A | 大項目（機能領域） | `029 申請管理機能` |
| B | 中項目 | `1.1 データ取込` |
| C | 小項目 | |
| D | 機能名称 | `申請データ取込` |
| F | **機能ID** | `0290001` |
| G | 要件文 | `・マイナポータルから申請ZIPファイルをダウンロードできること` |
| H | 実装必須／任意 | `実装必須機能` |

**機能IDはPDFとExcelの自然な結合キーである。** PDFの業務フローに `（機能 ID 0310005）` が18件あり、Excelの F列と同じ7桁の体系だった。**内部IDを新しく作る必要は無い。**
...
231 件  037 統合滞納管理
 ...
 24 件  032 団体内統合宛名機能
 25 件  034 EUC機能
 21 件  031 住登外者宛名番号管理機能
 11 件  030 庁内データ連携機能
```

要件文は総計65,431字、中央値61字、最大602字。

**「機能領域で絞って1リクエスト」は下位6領域でしか成立しない。** 実測の文字あたり1.080 state tokenで換算すると次のとおり（測り直しは [spec-budget.live.spec.ts](../src/lib/server/spec-budget.live.spec.ts)）。

| 対象 | 件数 | 推計 state token |
|---|---|---|
| 測定済みの上限 | 40 | — |
| 統合収納管理 | 419 | 約38,900（**32k超過**） |
| 統合滞納管理 | 231 | 約21,500 |
| Excel全体 | 761 | 約70,700（**32k超過**） |

統合収納管理は1領域だけで `state` と最長の質問の32k制限を超える。

**中項目まで絞り込みの階層に入れる。** 統合収納管理は13種類に分かれ、最大86件・中央値21件である。
...
ただし86件も測定済みの40件を超える。**3段でも足りるとは限らない。** 40 / 86 / 231 の3点を測り、崩れる場所を確認してから方式を決める。

#### 本文にルビが混入している

`sharedStrings` にふりがなが埋まっている。そのまま読むと壊れる。

```text
大項目  : 748/761 件   例「029 申請管理機能シンセイカンリキノウ」
機能名称: 451/761 件   例「申請データ取込シンセイトリコミ」
要件文  : 242/761 件
```

`<si>` から `<rPh>` を除いて取る。放置すると**Jevへ渡す要件文にも画面にも「キョウツウキノウ」が出る**。PDF側では起きない（`pdftotext` はルビを持たない）。

### 10.4 データモデル

PDFのcorpusへ混ぜず、**独立したdataset**として扱う。
...
type SpecRequirement = {
	...
	sourceUrl: string;
	version: string;
	/** ルビ除去などの正規化をしたか。PDL 1.0 の加工表示に使う。 */
	normalized: boolean;
};
```

**原資料に無いIDを作って公式IDのように見せない。** 機能IDがそのまま使えるため、内部IDは要らない。

### 10.5 検索方式は実測で決める

いきなり全行をJevへ送らない。ただし**二段階と決め打ちもしない**。

```text
A  PDF -> Jev -> 機能領域 -> 要件 -> Jev（2リクエスト）
B  決定的な絞り込み -> 要件 -> Jev（1リクエスト）
C  必要なsubsetを同一リクエストで評価
```

v0の実測で、分割はlatency 22倍・入力token 2.5倍になった。**設計上きれいな二段階より、実測のtoken / latency / 精度を優先する。**

### 10.6 Source Sufficiency

「関連箇所は見つかったが、この資料だけでは質問へ直接答えられない」状態を扱う。

```text
入力: 「ChatGPTで住民情報を分析していい？」
EUCやデータ連携に関連するpassageは存在する。
しかし共通機能標準仕様書だけで、生成AIへの個人情報入力可否は判断できない。
```

画面:

> 関連する仕様箇所は見つかりました。ただし、この資料だけでは質問に直接対応する記述を確認できませんでした。

**Jevのprobabilityだけで確定しない。** 資料の種別、検索結果、閾値を組み合わせ、実験として評価する。

### 10.7 Gold Dataset

v1の実装前に、現実的な質問を20件以上作る。各caseに人手でラベルを付ける。

```ts
expectedSource: 'main-spec' | 'functional-requirements' | 'other-spec' | 'out-of-scope';
```

可能なら期待する機能領域と機能IDも持たせ、**Retrieval / Corpus / Source の3つのGapを区別できる**ようにする。

**gold datasetはExcelの取り込みより先に作る。** Source Gapがどれだけあるかが分かれば、そもそもLayer 2が要るのか、要るならどの領域が効くのかを判断できる。761行の正規化とルビ処理は相応の作業なので、必要性を確かめてから着手する。

### 10.8 v0から変えないこと

- 全文を毎回送らない
- Jevに回答文を生成させない
- Jevに法的・行政的・仕様適合性の判断をさせない
- source metadataをJevの回答から作らない
- Jevのprobabilityを正答率・仕様適合率と表現しない

## 11. Excelを後回しにした理由（v0時点）
