...
BATCH JUDGE は `/batch` で、短文を1行1件で最大50件、**1回のリクエストで**まとめて判定します。処理の流れと結果のまとめも実測値で表示します。同梱の例文（`data/batch/*.json`、3テーマ各50件。**暫定goldラベル付き。人手確認前**）をワンクリックで入れられるので、自分の文章を貼らずに試せます。実測の結果は [BATCH_JUDGE_DESIGN.md](docs/BATCH_JUDGE_DESIGN.md) §4.6 にあります。
